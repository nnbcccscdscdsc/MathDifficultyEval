...
class DeepMath103KCustomDifficultyAnalyzer:
    """DeepMath-103K自定义难度分析器"""
    ...
    def __init__(self):
        ...
        # 数据路径
        self.data_path = Path("data/DeepMath-103K_result")
    ...
    def load_model_results(self, model_name: str) -> List[Dict]:
        ...
        # DeepMath-103K直接使用intermediate_results_*.json文件
        result_files = glob.glob(str(model_dir / "intermediate_results_*.json"))
        
        if not result_files:
            print(f"❌ 没有找到结果文件: {model_dir}")
            return []
    ...
    def create_custom_difficulty_plots(self, all_results: Dict[str, List[Dict]], custom_difficulties: Dict[str, Dict[str, str]]):
        ...
        fig.suptitle('DeepMath-103K Dataset - Custom Difficulty Analysis based on Model Performance', 
        ...
        # 保存图片
        plt_file = self.plot_dir / 'DeepMath_103K_custom_difficulty_comparison.png'
    ...
    def save_unified_results(self, all_results: Dict[str, List[Dict]], custom_difficulties: Dict[str, Dict[str, str]]):
        ...
        # 保存统一结果集
        unified_file = save_dir / "DeepMath103K_unified_results.json"
    ...
    def run_analysis(self):
        ...
        print("✅ DeepMath-103K自定义难度分析完成！")

def main():
    """主函数"""
    analyzer = DeepMath103KCustomDifficultyAnalyzer()
