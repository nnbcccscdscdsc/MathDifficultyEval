#!/usr/bin/env python3
"""
测试DeepSeek-R1-Distill-Qwen-1.5B数学模型
"""
...
def test_deepseek_r1_1_5b():
    """测试DeepSeek-R1-Distill-Qwen-1.5B模型"""
    model_name = "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"
    
    logger.info(f"🧮 测试DeepSeek-R1-Distill-Qwen-1.5B数学模型: {model_name}")
    ...
    try:
        ...
        for i, question in enumerate(test_questions, 1):
            ...
            with torch.no_grad():
                outputs = model.generate(
                    ...
                    max_new_tokens=400,  # 1.5B模型可以生成更长内容
                    do_sample=True,
            ...
            if "\\boxed{" in full_response:
                start = full_response.find("\\boxed{") + 1.5
        ...
        logger.error(f"❌ DeepSeek-R1-1.5B测试失败: {e}")
        import traceback
        traceback.print_exc()
        return ""

if __name__ == "__main__":
    test_deepseek_r1_1_5b() 